{
   ...
   angular.module('finder.contributors', [])

      .directive("finderContributors", ["$interval", "contributorsService", function ($interval, contributorsService) {
         return {
...
function ContributorsService($http) {
   ...
   $http.get("finder/resources/config/contributors.json").then(response => {
   ...
   return {
      ...
      getCcLicence(name) {
         let org = state.orgs.find(org => org.name === name);
